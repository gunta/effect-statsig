import { Layer, Effect, Data } from "effect"
import {
  Analytics,
  AnalyticsError,
  AnalyticsEvent,
...
export class BrowserAdapterError extends Data.TaggedError("BrowserAdapterError")<{
  readonly operation: string
  readonly cause?: unknown
}> {}

export interface BrowserStatsigClient {
...
const wrap = <A>(operation: string, run: () => Promise<A>) =>
  Effect.tryPromise({
    try: run,
    catch: (cause) => new BrowserAdapterError({ operation, cause })
  })

export const updateBrowserUser = (client: BrowserStatsigClient, user: StatsigUser) =>
  wrap("updateUser", () => client.updateUser(user))

export const makeBrowserServices = (client: BrowserStatsigClient) => {
...
export const makeBrowserLayer = (input: {
  readonly client: BrowserStatsigClient
  readonly config: StatsigConfig
  readonly user: StatsigUser
}) => {
  const services = makeBrowserServices(input.client)
